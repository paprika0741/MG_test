...
def check_hf(path):
    ...
    if config.num_local_experts % world_size == 0:
        ...
        base = config.num_local_experts // world_size
        extra = config.num_local_experts  % world_size
        num_local_experts = base + (1 if ep_rank < extra else 0)
        offset = sum(
            base + (1 if r < extra else 0) for r in range( ep_rank)
            )
        local_expert_indices = [offset + i for i in range( num_local_experts)]
    print(f"EP RANK {ep_rank} num_local_experts {num_local_experts} local_expert_indices {local_expert_indices}")
    # 打印调试信息
